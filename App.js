...
import { Text, View, ScrollView, TouchableOpacity, TextInput, Modal, Alert, SafeAreaView, StatusBar, Linking, Image, Share, StyleSheet } from 'react-native';
...
export default function App() {
  ...
  const openUpi = (app) => {
    ...
    const url = app === 'pp' ? `phonepe://pay?pa=${selectedUpi}&pn=SolarInvest&am=${amt}&cu=INR` : `paytmmp://pay?pa=${selectedUpi}&pn=SolarInvest&am=${amt}&cu=INR`;
    ...
    <SafeAreaView style={{flex:1, backgroundColor:'#0a101d'}}>
      ...
      <View style={{padding:15, flexDirection:'row', justifyContent:'space-between', backgroundColor:'#0e1726'}}>
        ...
        <TouchableOpacity onPress={openSupport}><Text style={{color:'#fff'}}>✈️ Support</Text></TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={{padding:20, paddingBottom:150}}>
        ...
        {bottomNav === 'Invite' && (
          <View style={{backgroundColor:'#1e293b', padding:20, borderRadius:15}}>
            ...
            <Text style={{color:'#fff', marginVertical:10}}>Ref: https://solarinvest.in/register?ref=Guri7412</Text>
            <TouchableOpacity style={{backgroundColor:'#16a34a', padding:15, borderRadius:10}} onPress={() => Share.share({message: 'Join: https://solarinvest.in/register?ref=Guri7412'})}><Text style={{color:'#fff', textAlign:'center'}}>🚀 Share Link</Text></TouchableOpacity>
          </View>
        )}
      </ScrollView>
      ...
      <View style={{position:'absolute', bottom:0, width:'100%', flexDirection:'row', justifyContent:'space-around', padding:15, backgroundColor:'#0e1726', borderTopWidth:1, borderColor:'#334155'}}>
        ...
        <TouchableOpacity onPress={() => setAdminVisible(true)}><Text style={{color:bottomNav==='Profile'?'#f59e0b':'#fff'}}>👤 Profile</Text></TouchableOpacity>
      </View>
      ...
      <Modal visible={modalVisible} transparent={true}><View style={{flex:1, backgroundColor:'#0008', justifyContent:'center', padding:20}}><View style={{backgroundColor:'#fff', padding:20, borderRadius:15}}>
        <TouchableOpacity onPress={() => setSelectedUpi('deepsingh7412@ibl')} style={{padding:10, borderWidth:1, margin:5}}>1. deepsingh7412@ibl</TouchableOpacity>
        <TouchableOpacity onPress={() => setSelectedUpi('mandeep7412@axl')} style={{padding:10, borderWidth:1, margin:5}}>2. mandeep7412@axl</TouchableOpacity>
