...
export default function App() {
  ...
  const openUpi = (app) => {
    ...
    Linking.openURL(targetUrl).catch(() => {
      Linking.openURL(generic).catch(() => Alert.alert('UPI App Error', 'Kripya UPI app install karein ya manual QR scan karein.'));
    });
  };
  ...
  const handleClaimIncome = (plan) => {
    if (plan.lastCollected === todayStr) {
      return Alert.alert('Already Collected', 'Aapne aaj ki income claim kar li hai. Kal dobara claim karein.');
    }
  ...
  const handleDepositSubmit = () => {
    if (!transactionId.trim() || transactionId.length < 6) {
      return Alert.alert('Required', 'Valid 12-digit UTR No. enter karein.');
    }
  ...
  const handleWithdrawSubmit = () => {
    if (!withdrawAmount || Number(withdrawAmount) < 200 || !withdrawUpi.trim() || Number(withdrawAmount) > balance) {
      return Alert.alert('Error', 'Details check karein (Min ₹200 aur balance hona chahiye).');
    }
    ...
    Alert.alert('Request Sent', '₹' + amt + ' withdrawal request submit ho gayi.');
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0a101d' }}>
      ...
      {/* Top Header */}
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingTop: Platform.OS === 'android' ? 36 : 14, paddingBottom: 14, backgroundColor: '#0e1726', borderBottomWidth: 1, borderColor: '#1e293b' }}>
        <Text style={{ fontSize: 19, fontWeight: 'bold', color: '#fff' }}>SOLAR <Text style={{ color: '#f59e0b' }}>INVEST</Text></Text>
      ...
      <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 110 }} showsVerticalScrollIndicator={false}>
        {bottomNav === 'Invest' && (
          <>
            ...
            <View style={{ backgroundColor: '#1e293b', borderRadius: 14, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#334155' }}>
              ...
              <Text style={{ fontSize: 34, fontWeight: 'bold', color: '#fff', marginVertical: 8 }}>₹{balance.toFixed(2)}</Text>
              <View style={{ flexDirection: 'row', gap: 10 }}>
                ...
                <TouchableOpacity style={{ flex: 1, backgroundColor: '#0284c7', borderRadius: 10, padding: 12, alignItems: 'center' }} onPress={() => setWithdrawModalVisible(true)}>
                  <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 13 }}>↗ Withdraw</Text>
                </TouchableOpacity>
              </View>
            </View>

            {/* Running Active Investments */}
            {myActivePlans.length > 0 && (
              <View style={{ marginBottom: 12 }}>
                ...
                {myActivePlans.map((ap) => {
                  ...
                  return (
                    <View key={ap.id} style={{ backgroundColor: '#162235', borderRadius: 10, padding: 12, marginBottom: 6, borderWidth: 1, borderColor: '#22c55e', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                      <View>
                        <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 14 }}>{ap.name}</Text>
                        <Text style={{ color: '#94a3b8', fontSize: 11, marginTop: 2 }}>Daily: <Text style={{ color: '#22c55e', fontWeight: 'bold' }}>+₹{ap.daily}</Text></Text>
                      </View>
                      <TouchableOpacity style={{ backgroundColor: isCollected ? '#334155' : '#16a34a', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 }} onPress={() => handleClaimIncome(ap)}>
                        <Text style={{ color: isCollected ? '#94a3b8' : '#fff', fontSize: 12, fontWeight: 'bold' }}>{isCollected ? '✅ Done' : '💰 Collect'}</Text>
                      </TouchableOpacity>
                    </View>
                  );
                })}
            ...
            {filtered.map((p) => (
              <View key={p.id} style={{ backgroundColor: '#1e293b', borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: '#334155' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                  <Text style={{ color: '#f59e0b', fontSize: 12, fontWeight: 'bold' }}>{p.badge}</Text>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  <View>
                    ...
                    <Text style={{ color: '#94a3b8', fontSize: 12, marginTop: 2 }}>Daily: <Text style={{ color: '#22c55e', fontWeight: 'bold' }}>₹{p.daily}</Text></Text>
                  </View>
                  <TouchableOpacity style={{ backgroundColor: '#f59e0b', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 }} onPress={() => handleInvest(p)}>
        ...
        {bottomNav === 'History' && (
          <View style={{ backgroundColor: '#1e293b', borderRadius: 14, padding: 14 }}>
            <Text style={{ color: '#fff', fontSize: 15, fontWeight: 'bold', marginBottom: 8 }}>Transaction History</Text>
            {history.map((h) => (
              <View key={h.id} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#334155' }}>
                <View><Text style={{ color: '#fff', fontSize: 13 }}>{h.type}</Text><Text style={{ color: '#64748b', fontSize: 10, marginTop: 2 }}>{h.date}</Text></View>
                <View style={{ alignItems: 'flex-end' }}>
                  <Text style={{ fontSize: 13, fontWeight: 'bold', color: h.amount.includes('+') ? '#22c55e' : '#fff' }}>{h.amount}</Text>
                  <Text style={{ color: h.status.includes('Pending') || h.status.includes('Review') ? '#f59e0b' : '#94a3b8', fontSize: 10, marginTop: 2 }}>{h.status}</Text>
                </View>
              </View>
            ))}
          </View>
        ...
        {bottomNav === 'Invite' && (
          <View style={{ backgroundColor: '#1e293b', borderRadius: 14, padding: 16 }}>
            <Text style={{ color: '#f59e0b', fontSize: 12, fontWeight: 'bold' }}>🎉 30% BONUS COMMISSION</Text>
            <Text style={{ color: '#fff', fontSize: 16, fontWeight: 'bold', marginVertical: 6 }}>Invite Friends & Earn 30% Direct</Text>
            <Text style={{ color: '#94a3b8', fontSize: 12, marginBottom: 10 }}>Apne referral link ko dosto ke sath share karein aur unke recharge par turant 30% direct commission paayein:</Text>
            <View style={{ backgroundColor: '#0f172a', padding: 12, borderRadius: 8, borderWidth: 1, borderColor: '#334155', marginBottom: 12 }}>
              <Text style={{ color: '#22c55e', fontWeight: 'bold', fontSize: 12 }}>{refLink}</Text>
            </View>
            <TouchableOpacity style={{ backgroundColor: '#16a34a', padding: 13, borderRadius: 10, alignItems: 'center' }} onPress={() => Share.share({ message: '☀️ Join Solar Invest & start earning daily! Link: ' + refLink })}>
              <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 13 }}>🚀 Share Invite Link (WhatsApp/SMS)</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {bottomNav === 'Profile' && (
          <View style={{ backgroundColor: '#1e293b', borderRadius: 14, padding: 16 }}>
            <Text style={{ color: '#fff', fontSize: 16, fontWeight: 'bold', marginBottom: 10 }}>User Dashboard</Text>
            <Text style={{ color: '#e2e8f0', fontSize: 13, marginBottom: 6 }}>Total Balance: ₹{balance.toFixed(2)}</Text>
            <Text style={{ color: '#e2e8f0', fontSize: 13, marginBottom: 14 }}>Active Running Plans: {myActivePlans.length}</Text>
            <TouchableOpacity style={{ backgroundColor: '#f59e0b', padding: 13, borderRadius: 10, alignItems: 'center', marginBottom: 10 }} onPress={() => setAdminModalVisible(true)}>
              <Text style={{ color: '#000', fontWeight: 'bold', fontSize: 13 }}>👑 Owner / Admin Panel ({depositRequests.length + withdrawRequests.length})</Text>
            </TouchableOpacity>
            <TouchableOpacity style={{ backgroundColor: '#0284c7', padding: 12, borderRadius: 10, alignItems: 'center' }} onPress={openSupport}>
              <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>✈️ 24/7 Official Support (@Guri7412)</Text>
            </TouchableOpacity>
          </View>
        )}
      </ScrollView>

      {/* 4 Bottom Fixed Navigation Tabs */}
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.85)', justifyContent: 'center', alignItems: 'center', padding: 16 }}>
          <View style={{ backgroundColor: '#fff', borderRadius: 18, padding: 16, width: '100%', maxWidth: 350 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 6, marginBottom: 6 }}>
              <TouchableOpacity style={{ flex: 1, backgroundColor: '#5f259f', padding: 8, borderRadius: 8, alignItems: 'center' }} onPress={() => openUpi('phonepe')}>
                <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>🟣 PhonePe</Text>
              </TouchableOpacity>
              <TouchableOpacity style={{ flex: 1, backgroundColor: '#00baf2', padding: 8, borderRadius: 8, alignItems: 'center' }} onPress={() => openUpi('paytm')}>
                <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>🔵 Paytm</Text>
              </TouchableOpacity>
            </View>
            ...
            <TouchableOpacity style={{ backgroundColor: '#16a34a', borderRadius: 8, padding: 10, alignItems: 'center', marginTop: 6 }} onPress={handleDepositSubmit}>
              <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 12 }}>Submit Recharge</Text>
